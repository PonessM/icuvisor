...
func createWorkoutInputSchema() map[string]any {
	...
	return map[string]any{"type": "object", "additionalProperties": false, "required": []string{"name", "folder_id", "sport"}, "examples": examples, "input_examples": examples, "properties": map[string]any{
		...
		"workout_doc": map[string]any{"type": "object", "description": "Optional structured WorkoutDoc for the initial upstream workout description/DSL. Serialized and merged with description when both are supplied. In each structured step, description is a label/comment only: do not include duration or distance tokens there; use duration seconds or distance instead. Set press_lap:true on a timed/distance step to serialize the documented Press lap control; do not put that marker in description. Zone targets are serialized using the supplied sport and the athlete's sport settings, adding metric suffixes such as Z2 Power, Z2 HR, or Z2 Pace when needed. Syntax reference: icuvisor://workout-syntax."},
