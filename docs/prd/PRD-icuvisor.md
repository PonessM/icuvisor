...
- `get_athlete_profile` — FTP, zones, sport settings, thresholds. Sport rows expose `ftp_watts`, optional `indoor_ftp_watts`, and `p_max_watts` as integer watts; `w_prime_joules` as integer joules; and `lthr_bpm` and `max_hr_bpm` as integer beats per minute. `power_zones_percent_of_ftp` is the exact upstream `[]integer` of positive, strictly increasing percentage upper ceilings, while `power_zones_watts` is the matching `[]number` derived as `ftp_watts * percent / 100`. `power_zone_names` aligns by index only when it has the same length. These arrays are zone boundaries, not alternate FTPs; absent/invalid inputs do not synthesize watt ceilings. `hr_zones_bpm` remains an upstream `[]integer` boundary array, while `pace_zones_percent_of_threshold` is an upstream `[]number` of percentage boundaries, never durations. Upstream `threshold_pace` is stored in meters per second; the response derives one explicit seconds-per-distance field solely from display-only `pace_units`. `_meta.warnings` uses stable power-setting codes: `missing_power_threshold`, `missing_power_zones`, `invalid_power_zone_ceilings`, and `mismatched_power_zone_names`; analyzers also use `missing_power_ftp` when normalization cannot derive watt ceilings. Response metadata states these conventions so assistants do not infer a separate indoor FTP from a zone boundary or planned-event Indoor flag.
- `update_sport_settings` — write outdoor FTP, indoor FTP in watts, threshold HR, threshold pace, and per-sport zone definitions back to the athlete profile (forum #35 — assistants offered FTP updates that didn't land because there was no write path). Indoor FTP is a sparse threshold-only write, independent of the outdoor FTP value; no indoor-versus-outdoor ordering rule is inferred. Threshold-pace inputs use explicit athlete-facing duration units but are transported as upstream m/s; `pace_units` is display metadata and `pace_load_type` preserves or disambiguates Run/Swim handling. Pace-zone boundaries are percent-of-threshold values, never durations. For `zones[].kind="power"`, `boundaries` are positive, strictly increasing integer percentage upper ceilings of FTP—not watts. Names, when supplied, must be a nonempty one-for-one array; omission preserves existing names only when they are absent or have the same count, while an existing nonempty count mismatch is rejected before HTTP and requires complete supplied names. This is an intentional write-contract break correcting earlier watt-boundary documentation; cached-schema clients must start a new conversation after upgrading. Per-sport scoped (`sport: "Ride" | "Run" | …`). Supplying any `zones` replaces prior definitions and is gated by `ICUVISOR_DELETE_MODE=full`, since a bad zone overwrite silently miscolours every historical activity.
...
- `compute_zone_energy` — full-toolset, read-only timestamp integration of recorded power into seconds and kJ of external mechanical work per configured athlete power zone over a bounded date range. It normalizes the configured power-zone percentage upper ceilings against FTP into watt ceilings, attributes configured named zones as `[lower, upper)`, and emits an explicit above-final overflow bucket for power at or above the final derived ceiling. Its pinned formula ref is `icuvisor://analysis-formulas#power_zone_mechanical_work_v2`. It uses canonical power/time streams, skips and reports invalid or missing intervals and activities with stable analyzer codes `missing_power_zones`, `missing_power_ftp`, `invalid_power_zone_ceilings`, and `mismatched_power_zone_names`, and never labels mechanical work as metabolic energy, calorie expenditure, or food calories.
