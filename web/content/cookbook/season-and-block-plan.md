...
include_per_sport_load_trends:true, report its explicit unavailable reason, and
use get_training_summary sport rows only for factual weekly run, bike, swim, and
other load totals when present. Do not derive per-sport CTL, ATL, TSB, fatigue,
or daily load from weekly category buckets. Summarize my current combined
fitness/form, my recent weekly load range, the partial-week limitation, and how
much training history you can actually see. Do not propose a plan yet.
```
